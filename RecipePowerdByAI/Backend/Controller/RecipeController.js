...
const getAllFavoriteRecipes = async (req, res) => {
  try {
    ...
    return res
      ...
      .send({
        error,
        message: "Unexpected error at retreaving favorite recipes",
      });
  }
};
...
const createFavoriteRecipe = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ recipe: recipe, message: "Recipe was added succesfully" });
  } catch (error) {
    console.log(error);
    return res
      .status(400)
      .send({
        error: error,
        message: "Unexpected error at creating a new favorite recipe",
      });
  }
};
...
const deleteFavoriteRecipe = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send({ error, message: "Unexpected error at deleting recipe" });
  }
};
